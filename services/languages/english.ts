...
export async function englishToIPA(text: string, useDictionary: boolean = true): Promise<string> {
    ...
    const results = await Promise.all(tokens.map(async (token) => {
        ...
        if (token.includes('-')) {
             ...
             const resolved = await Promise.all(parts.map(p => predict(p, useDictionary)));
             return resolved.join('');
        }
        let pron = await predict(token, useDictionary);
